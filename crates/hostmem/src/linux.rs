...
/// A page-aligned page-sized buffer: what `O_DIRECT` requires of user
/// memory.
pub struct PageBuf {
    ptr: *mut u8,
}
...
/// A local segment file opened `O_DIRECT` — the mechanism the production
/// write path uses: page I/O straight between our buffers and the device,
/// no page-cache copy of the data anywhere.
pub struct DirectFile {
    file: std::fs::File,
}

impl DirectFile {
    pub fn create(path: &str) -> io::Result<DirectFile> {
    ...
    /// `O_DIRECT` bypasses the page cache for data; fsync still pins down
    /// metadata and the device cache.
