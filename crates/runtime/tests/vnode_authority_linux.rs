...
#[test]
fn fsynced_generation_and_closure_survive_reopen_and_a_torn_tail() {
    let root = test_root();
    if root.exists() {
    ...
    let blobs = FileBlobs::new(&root).expect("file blobs");
    ...
    let reopened = FileBlobs::new(&root).expect("reopen file blobs");
