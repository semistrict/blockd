...
pub fn install_replica_recovery(
    target: &Path,
    ...
    let head_version = store
        ...
        .map_err(|_| InstallReplicaRecoveryError::ClaimConflict)?;
    durable_promote(target, vset, writer_fence, &export.blobs)?;
...
mod tests {
    ...
    impl ObjectStore for MemoryStore {
        fn put(&self, key: &str, bytes: Vec<u8>) -> Result<u64, StoreFault> {
            ...
            objects.insert(key.to_owned(), (version, bytes));
            Ok(version)
        }
        ...
        fn get_range(&self, _key: &str, _offset: u64, _len: u64) -> crate::store::GetResult {
            unreachable!()
        }
    ...
    #[test]
    fn promotion_claims_refences_publishes_then_atomically_installs() {
        let vset = VsetId(7);
        ...
        let installed = install_replica_recovery(&target, &store, HostId(2), vset, 1, &export)
            .expect("install");
    ...
    #[test]
    fn promotion_rejects_an_export_verified_against_an_older_head() {
        let vset = VsetId(8);
        let store = MemoryStore::default();
        ...
        let _ = std::fs::remove_dir_all(&target);
        let result = install_replica_recovery(&target, &store, HostId(2), vset, 1, &export);
        if target.exists() {
