...
#[test]
fn journal_v4_golden_bytes_decode() {
    let (vset, record) = golden_journal_record();
    let golden = journal_golden();
    assert_eq!(hex(&record.encode(vset)), golden, "encoder drifted");
    assert_eq!(
        JournalRecord::decode(vset, &unhex(golden)),
        Ok(record),
