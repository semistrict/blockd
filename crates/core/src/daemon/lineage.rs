...
/// One in-flight base keep: segments to copy, then the record.
#[derive(Debug)]
...
impl Daemon {
    ...
    pub(super) fn keep_step(&mut self, vset: VsetId, out: &mut Vec<Effect>) {
        ...
        // All segments copied: publish the record with own-namespace
        // locations rewritten to the base's.
        let base = keep.base;
        let req = keep.req;
        let mut record = keep.record.clone();
        for (_, loc) in record.pages.values_mut() {
            if loc.base == 0 {
                loc.base = base;
            }
        }
    ...
    pub(super) fn fork_base_done(
        ...
        // O(1) fork (R5.1): re-key the base's pages under the new vset;
        // every location still points into the base namespace (R5.3).
        let mut pages: BTreeMap<PageId, _> = BTreeMap::new();
        let whole = matches!(record.kind, RecordKind::Checkpoint { .. });
        for (page, entry) in &record.pages {
