...
async fn retry(state: &SharedHost) {
    state.borrow_mut().counters.store_retries += 1;
    let delay_ns = state.borrow().config.backup_retry;
    ...
    loop {
        match Store::put_cas(world, key.clone(), None, bytes.clone()).await {
            ...
            Err(StoreError::Fault(crate::protocol::StoreFault::CasConflict { actual })) => {
                let found = get(state, world, &key).await?;
                return match found {
...
pub async fn read<W: Store>(state: &SharedHost, world: &W, key: &str) -> Option<Vec<u8>> {
    get(state, world, key)
        .await
        .ok()
        .flatten()
...
pub async fn write<W: Store>(
    ...
    put(state, world, key, bytes).await.ok()
}
...
pub async fn write_immutable<W: Store>(
    ...
    put_immutable(state, world, key, bytes).await.ok()
}
